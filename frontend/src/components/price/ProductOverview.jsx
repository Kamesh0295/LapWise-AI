...
const ProductOverview = ({ laptop, summary, onOpenAlertModal }) => {
  ...
  return (
    <div className="p-6 sm:p-8 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-black/20 space-y-6">
      
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        
        {/* Left Column: Image Gallery */}
        <div className="lg:col-span-5 flex flex-col items-center gap-4">
          <div className="w-full h-64 sm:h-72 flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
            <img 
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap text-xs font-bold">
            <span className="px-3 py-1 bg-primary-500/10 text-primary-600 dark:bg-primary-500/20 dark:text-primary-400 rounded-full uppercase tracking-wider">
              {laptop.brand} {laptop.series ? `• ${laptop.series}` : ''}
            </span>

            <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-500/10 text-green-700 dark:text-green-400 rounded-full border border-green-500/20">
              <MdEmojiEvents className="text-amber-500" size={14} />
              <span>Lowest Price</span>
            </span>

            <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-500/10 text-green-700 dark:text-green-400 rounded-full border border-green-500/20">
              <MdLocalOffer className="text-green-600 dark:text-green-400" size={14} />
              <span>Best Deal</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
            <div className="flex items-center text-amber-500 font-bold">
              ...
              <span className="text-gray-500 dark:text-gray-400 ml-1">({laptop.reviewCount || laptop.numReviews || 167} reviews)</span>
            </div>
            <span>•</span>
          ...
          <div className="p-5 bg-[#EBF5EA] dark:bg-gray-800/90 border border-green-200/60 dark:border-green-800/40 rounded-2xl flex flex-wrap items-baseline justify-between gap-4">
            <div>
              <span className="text-xs font-bold text-gray-700 dark:text-gray-300 block">Current Lowest Price</span>
              <span className="font-outfit text-3xl sm:text-4xl font-black text-[#009944] dark:text-green-400">
                ₹{lowestPrice.toLocaleString('en-IN')}
              </span>
            </div>

            <div className="text-right space-y-0.5">
              <span className="text-xs text-gray-500 dark:text-gray-400 block">Highest Available: ₹{highestPrice.toLocaleString('en-IN')}</span>
              <span className="text-xs font-black text-[#009944] dark:text-green-400 block">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <button
              ...
              className="px-5 py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-outfit text-xs font-extrabold rounded-xl shadow-sm flex items-center gap-2 transition-all hover:scale-105"
            >
            ...
            <button
              ...
              className={`px-4 py-2.5 border rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all ${
                inCompare
                  ? 'border-primary-500 text-primary-500 bg-primary-50 dark:bg-primary-950/30'
                  : 'border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'
              }`}
            >
